...
def run_daily_ingest(
    ...
    # デバッグ: InfluxDB接続情報をログ出力（ERRORレベルで強制表示）
    logger.error(f"[DEBUG] InfluxDB接続情報: host={config.host}, org={config.org}, token=...{config.token[-10:] if config.token else 'None'}")
    logger.error(f"[DEBUG] InfluxDBバケット設定: raw_1m_hot={config.bucket_raw_1m_hot}, agg_5m={config.bucket_agg_5m}, agg_1d={config.bucket_agg_1d}")
    ...
    with InfluxDBClient3(
        ...
        for interval, days in interval_days.items():
            ...
            # デバッグ: 書き込み対象バケット情報をログ出力（ERRORレベルで強制表示）
            logger.error(f"[DEBUG] interval={interval}: バケット={bucket} に書き込み開始")
